...
class Command:
    ...
    actions = {'move': ['jump', 'walk', 'run', 'crouch', 'turn']}  # planned supported actions for status report
    ...
    def check_adj(self, word):
        newWord = word.text
        rightChildTokens = [tok.text for tok in word.rights if
                            tok.pos_ == "NUM" or tok.pos_ == "ADJ" or tok.dep_ == "compound"]
        leftChildTokens = [tok.text for tok in word.lefts if
                           tok.pos_ == "NUM" or tok.pos_ == "ADJ" or tok.dep_ == "compound"]
        ...
        for token in self.doc[3:]:
            if token.pos_ == 'VERB':
                ...
                for child in token.children:
                    ...
                    if child.text == 'right':
                        pair[str(token.lemma_) + ' right'] = pair.pop(token.lemma_)
                    elif child.text == 'left':
                        pair[str(token.lemma_) + ' left'] = pair.pop(token.lemma_)
                    elif child.text == 'backward' or child.text == 'back':
                        pair[str(token.lemma_) + ' backward'] = pair.pop(token.lemma_)
                    elif child.text == 'forward':
                        pair[str(token.lemma_) + ' forward'] = pair.pop(token.lemma_)
